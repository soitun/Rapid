...
function Action_rapid(ev, appId, pageId, controlId, actionId, actionType, successCallback, errorCallback) {
	...
	switch (actionType) {
		...
		break;
		case "NEWROLE" :		
			data = { actionType: actionType, appId: $("#rapid_P0_C43").val(), role: $("#rapid_P5_C7").val() };
			callback = function() {
				Event_change_rapid_P0_C43(ev);
			};								
		break;
		case "DELROLE" :		
			data = { actionType: actionType, appId: $("#rapid_P0_C43").val(), role: $(ev.target).parent().prev().html() };
			callback = function() {
				Event_change_rapid_P0_C43(ev);
			};								
		break;
		case "NEWUSER" :		
			data = { actionType: actionType, appId: $("#rapid_P0_C43").val(), userName: $("#rapid_P6_C7").val(), password: $("#rapid_P6_C18").val() };
			callback = function() {
				Event_change_rapid_P0_C43(ev);
			};								
		break;
		case "DELUSER" :		
			data = { actionType: actionType, appId: $("#rapid_P0_C43").val(), userName: $(ev.target).parent().prev().html() };
			callback = function() {
				Event_change_rapid_P0_C43(ev);
			};								
		break;
		...
		case "SAVEUSERPASSWORD" :	
			data = { actionType: actionType, appId: $("#rapid_P0_C43").val(), userName: $("#rapid_P0_C216").find("tr.rowSelect").children().first().html(), password: $("#rapid_P0_C231").val() };															
		break;
		case "TESTDBCONN" :	
			data = { 
				actionType: actionType, 
