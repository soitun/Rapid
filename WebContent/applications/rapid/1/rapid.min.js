...
m=function(){$("#rapid_P0_C81").change();$("#rapid_P0_C74").click()};break;case"NEWUSER":if(j){f={actionType:d,appId:"rapid",userName:$("#rapid_P16_C8_").val(),description:$("#rapid_P16_C13_").val(),password:$("#rapid_P16_C17_").val(),useAdmin:$("rapid_P16_C38_").prop("checked"),useDesign:$("rapid_P16_C39_").prop("checked")}}else{f={actionType:d,appId:$("#rapid_P0_C43").val(),version:$("#rapid_P0_C1044_").val(),userName:$("#rapid_P6_C7").val(),description:$("#rapid_P6_C42_").val(),password:$("#rapid_P6_C18").val()};m=function(){Event_change_rapid_P0_C43(k)}}break;case"DELUSER":if(j){f={actionType:d,appId:"rapid",userName:$(k.target).closest("tr").find("td").first().html()};m=function(){$("#rapid_P0_C827_").hide();Action_rapid(k,"rapid","P0",null,"P0_A901_","GETUSERS",true)}}else{f={actionType:d,appId:$("#rapid_P0_C43").val(),version:$("#rapid_P0_C1044_").val(),userName:$(k.target).closest("tr").find("td").first().html()};m=function(){$("#rapid_P0_C81").change();$("#rapid_P0_C74").click()
}}break;case"NEWUSERROLE":f={actionType:d,appId:$("#rapid_P0_C43").val(),version:$("#rapid_P0_C1044_").val(),userName:$("#rapid_P0_C216").find("tr.rowSelect").children().first().html(),role:$("#rapid_P0_C254").val()};m=function(){Event_click_rapid_P0_C216({target:$("#rapid_P0_C216").find("tr.rowSelect").children().first()[0]})};break;case"DELUSERROLE":f={actionType:d,appId:$("#rapid_P0_C43").val(),version:$("#rapid_P0_C1044_").val(),userName:$("#rapid_P0_C216").find("tr.rowSelect").children().first().html(),role:$(k.target).parent().prev().html()};m=function(){Event_click_rapid_P0_C216({target:$("#rapid_P0_C216").find("tr.rowSelect").children().first()[0]})};break;case"SAVEUSER":if(j){f={actionType:d,appId:"rapid",version:$("#rapid_P0_C1044_").val(),userName:$("#rapid_P0_C823_").find("tr.rowSelect").children().first().html(),description:$("#rapid_P0_C838_").val(),password:$("#rapid_P0_C843_").val(),useAdmin:$("#rapid_P0_C879_").prop("checked"),useDesign:$("#rapid_P0_C880_").prop("checked")}
}else{f={actionType:d,appId:$("#rapid_P0_C43").val(),userName:$("#rapid_P0_C216").find("tr.rowSelect").children().first().html(),description:$("#rapid_P0_C717_").val(),password:$("#rapid_P0_C231").val()};m=function(){$("#rapid_P0_C81").change();$("#rapid_P0_C74").click()}}break;case"NEWPARAM":f={actionType:d,appId:$("#rapid_P0_C43").val(),version:$("#rapid_P0_C1044_").val()};break;case"DELPARAM":f={actionType:d,appId:$("#rapid_P0_C43").val(),version:$("#rapid_P0_C1044_").val(),index:$(k.target).closest("tr").index()-1};break;case"SAVEPARAM":f={actionType:d,appId:$("#rapid_P0_C43").val(),version:$("#rapid_P0_C1044_").val(),index:$("#rapid_P0_C1108_").find("tr.rowSelect").index()-1,name:$("#rapid_P0_C1134_").val(),value:$("#rapid_P0_C1123_").val()};break;case"TESTDBCONN":f={actionType:d,appId:$("#rapid_P0_C43").val(),version:$("#rapid_P0_C1044_").val(),index:$("#rapid_P0_C311").find("tr.rowSelect").index()-1,driver:$("#rapid_P0_C338").val(),connectionString:$("#rapid_P0_C374").val(),connectionAdapter:$("#rapid_P0_C339").val(),userName:$("#rapid_P0_C340").val(),password:$("#rapid_P0_C341").val()};
...
}}if(p){m.push(f);break}}if(m.length==k){m.push("")}if(a.columns[k].cellFunction){a.columns[k].cellFunction=new Function(a.columns[k].cellFunction)}}for(var k in h.rows){var o=h.rows[k];var l=g.append("<tr class='rowStyle"+(k%2+1)+"'></tr>").find("tr:last");for(var f in a.columns){var b="";if(!a.columns[f].visible){b+="display:none;"}if(a.columns[f].style){b+=a.columns[f].style}if(b){b=" style='"+b+"'"}var c=l.append("<td"+b+">"+((m[f])?o[m[f]]:"")+"</td>").find("td:last");if(a.columns[f].cellFunction){a.columns[f].cellFunction.apply(c,[d,h,n,a])}}}}else{for(var k in h.rows){var o=h.rows[k];var e="<tr>";for(var f in o){e+="<td>"+o[f]+"</td>"}e+="</tr>";g.append(e)}}}g.children().last().children("tr:not(:first)").click(function(){var i=$(this);i.parent().find("tr.rowSelect").each(function(){var j=$(this);j.removeClass("rowSelect")});i.addClass("rowSelect")})}}function getProperty_grid_columnCount(b,d,c,a){return($("#"+d).find("tr").first().children("td").size())}function getProperty_grid_rowCount(b,d,c,a){return($("#"+d).find("tr").size()-1)
}function getData_input(b,d,c,a){return $("#"+d).val()}function setData_input(f,c,e,b){var d=$("#"+f);if(c!=null&&c!==undefined){c=makeDataObject(c,e);if(c.rows&&c.rows[0]){if(e&&c.fields&&c.fields.length>0){for(var a in c.fields){if(c.fields[a].toLowerCase()==e.toLowerCase()){d.val(c.rows[0][a]);break}}}else{if(c.rows[0][0]){d.val(c.rows[0][0])}else{d.val("")}}}else{d.val("")}}else{d.val("")}}function getData_radiobuttons(b,d,c,a){return $("#"+d).children("input[type=radio]:checked").val()}function setData_radiobuttons(g,e,f,b){if(e!=null&&e!==undefined){var d=$("#"+g);d.children("input[type=radio]").prop("checked",false);e=makeDataObject(e,f);var c=null;if(e.rows&&e.rows[0]){if(f&&e.fields){for(var a in e.fields){if(e.fields[a].toLowerCase()==f.toLowerCase()){c=e.rows[0][a];break}}}else{c=e.rows[0][0]}}if(c){d.children("input[type=radio][value="+c+"]").prop("checked",true)}}}function getData_text(b,d,c,a){return $("#"+d).html()}function setData_text(f,c,e,b){var d=$("#"+f);if(c!=null&&c!==undefined){c=makeDataObject(c,e);
