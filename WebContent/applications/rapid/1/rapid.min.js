...
var n=-1;for(var a in f.fields){if(m.toLowerCase()==f.fields[a].toLowerCase()){n=a;break}}if(n>-1){p.push(c[n])}else{p.push(null)}}d.rows.push(p)}}else{d.rows=[];d.rows.push([])}}else{d.fields=[];d.rows=[];d.rows.push([]);for(var e in g){var l=g[e];d.fields.push(l.id);d.rows[0].push(l.value)}}return d}var _webserviceActionSequence=[];var _webserviceActionMaxSequence=[];function getWebserviceActionSequence(b){var a=_webserviceActionSequence[b];if(!a){a=0}a++;_webserviceActionSequence[b]=a;return a}function getWebserviceActionMaxSequence(b){var a=_webserviceActionMaxSequence[b];if(a===undefined){a=0;_webserviceActionMaxSequence[b]=a}return a};function Init_grid(g,c){var e=$("#"+g);if(c){if(c.columns){var f=e.find("tr:first-child td");for(var b in c.columns){if(c.columns[b].sort){var a=$(f.get(b));a.css("cursor","pointer");a.append("<span class='sort'>&#xf0dc;</span>");a.click({index:b},function(i){var h=$(i.target);if(!h.is("span")){h=h.find("span")}if(h.html()=="\uf0de"){sortGridByColumn(i,g,c,i.data.index,false);h.html("&#xf0dd;")}else{sortGridByColumn(i,g,c,i.data.index,true);h.html("&#xf0de;")}})}}}if(c.dataStorageType){var d=getGridDataStoreData(g,c);if(d){setData_grid($.Event("gridinit"),g,null,c,d)}$("#"+g).click(function(h){var i=getGridDataStoreData(g,c);i.selectedRowNumber=$(h.target).closest("tr").index();saveGridDataStoreData(g,c,i)})}}}function Init_hints(f,d){var a=$("body");for(var c in d.controlHints){var b=d.controlHints[c];if(!$("#"+b.controlId+"hint")[0]){var e=b.style;if(e){e=" style='"+e+"'"}else{e=""}a.append("<span class='hint' id='"+b.controlId+"hint'"+e+">"+b.text+"</span>");$("#"+b.controlId+"hint").hide()
}$("#"+b.controlId).mouseout({controlId:b.controlId},function(g){$("#"+g.data.controlId+"hint").hide()});switch(b.type){case"click":$("#"+b.controlId).click({controlId:b.controlId},function(g){$("#"+g.data.controlId+"hint").css({left:g.pageX+5,top:g.pageY+5}).show()});break;case"hover":$("#"+b.controlId).mouseover({controlId:b.controlId},function(g){$("#"+g.data.controlId+"hint").css({left:g.pageX+5,top:g.pageY+5}).show()});break}}}function Init_pagePanel(c,b){var a="<center><h1>Page</h1></center>";$.ajax({url:"~?a="+_appId+"&v="+_appVersion+"&p="+b.pageId+"&action=dialogue",type:"GET",data:null,error:function(g,d,f){var e="Error loading page : "+f},success:function(h){if(h){var k=$("head");a="";script="";var j=$(h);for(var f in j){switch(j[f].nodeName){case"#text":case"TITLE":case"META":break;case"SCRIPT":if(j[f].innerHTML){var m=j[f].outerHTML;if(m.indexOf("var _appId =")==-1&&m.indexOf("var _appVersion =")==-1&&m.indexOf("var _pageId =")==-1&&m.indexOf("var _mobileResume =")==-1){script+=m
