...
if(h>=0){for(var g in b.columns){if(b.columns[g].field&&b.columns[g].field.toLowerCase()==m.toLowerCase()){f=o.children(":nth("+g+")").html();break}}}if(f==null&&b.dataStorageType){var a=getGridDataStoreData(c,b);if(a.rows>h){for(var g in a.fields){if(a.fields[g]&&a.fields[g].toLowerCase()==m.toLowerCase()){f=a.rows[h][g];break}}}}}else{var f={};f.fields=[];for(var g in b.columns){var m=b.columns[g].field;if(m){f.fields.push(b.columns[g].field)}}f.rows=[];var n=$("#"+c);if(!n.is("table")){n=n.find("table").last()}n.find("tr:not(:first):not([data-ignore])").each(function(j){var p=[];$(this).children().each(function(q){if(b.columns[q].field){p.push($(this).html())}});f.rows.push(p)});if(b.dataStorageType){var a=getGridDataStoreData(c,b);if(a){var d={};for(var g in f.fields){for(var e in a.fields){if(f.fields[g]&&a.fields[e]&&f.fields[g].toLowerCase()==a.fields[e].toLowerCase()){d[g]=e;break}}}for(var g in f.rows){if(a.rows.length<g-1){a.rows.push([]);for(k in a.fields){a.rows[g].push(null)
