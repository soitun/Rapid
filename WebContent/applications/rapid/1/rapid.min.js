...
break}}}}else{var c={};c.fields=[];for(var d in a.columns){var g=a.columns[d].field;if(g){c.fields.push(a.columns[d].field)}}c.rows=[];var h=$("#"+b);if(!h.is("table")){h=h.find("table").last()}h.find("tr:not(:first):not([data-ignore])").each(function(k){var l=[];$(this).children().each(function(m){if(a.columns[m].field){l.push($(this).html())}});c.rows.push(l)})}}}}return c}function setData_grid(v,n,a,u,y,m){var f=$("#"+n);f.find("tr:not(:first-child)").remove();f.find("span.sort").html("&#xf0dc");if(!f.is("table")){f=f.find("table").last()}if(y!==undefined){y=makeDataObject(y,a);if(y&&y.rows){if(u&&u.columns&&y.fields){var d=[];for(var s in u.columns){for(var q in y.fields){var k=false;if(u.columns[s].field){if(u.columns[s].field.toLowerCase()==y.fields[q].toLowerCase()){k=true}}else{if(!y.fields[q]){k=true}}if(k){d.push(q);break}}if(d.length==s){d.push("")}if(u.columns[s].cellFunction&&!u.columns[s].f){u.columns[s].f=new Function(u.columns[s].cellFunction)}}for(var s in y.rows){var e=y.rows[s];
var h=f.append("<tr class='rowStyle"+(s%2+1)+"'></tr>").find("tr:last");for(var q in u.columns){var t="";if(!u.columns[q].visible){t+="display:none;"}if(u.columns[q].style){t+=u.columns[q].style}if(t){t=" style='"+t+"'"}var p="";var o=d[q];if(o&&e[o]){p=e[d[q]]}var g=h.append("<td"+t+">"+p+"</td>").find("td:last");if(u.columns[q].f){u.columns[q].f.apply(g,[n,y,a,u])}}if(y.rowValidation&&s<y.rowValidation.length){if(!y.rowValidation[s]){h.addClass("validation")}}}if(y.selectedRowNumber){f.find("tr:nth("+y.selectedRowNumber+")").addClass("rowSelect")}}else{for(var s in y.rows){var e=y.rows[s];var w="<tr>";for(var q in e){w+="<td>"+e[q]+"</td>"}w+="</tr>";f.append(w)}}}if(u&&u.dataStorageType){saveGridDataStoreData(n,u,y)}if(u&&u.fixedHeader){var r=false;if(!f.is(":visible")){r=true;f.parent().show()}var x=f.prev().find("tr:first-child").children();x.css("width","");var b=f.find("tr:nth-child(2)").children();if(b.length>0){for(var s=0;s<b.length;s++){var c=$(x.get(s));var l=$(b.get(s));if(c.width()<l.width()){c.width(l.width())
}if(c.width()>l.width()){l.width(c.width())}}}if(r){f.parent().hide()}}f.children().last().children("tr:not(:first)").click(function(){var i=$(this);i.parent().find("tr.rowSelect").removeClass("rowSelect");i.addClass("rowSelect")})}}function getProperty_grid_selectedRowData(d,h,f,c){var e=null;if(c.dataStorageType){e=getGridDataStoreData(h,c);e.rows=[e.rows[e.selectedRowNumber-1]]}else{var g=$(d.target).closest("tr");var a=g.index();if(a>0){e={fields:[],rows:[[]]};for(var b in c.columns){e.fields.push(c.columns[b].field);e.rows[0].push(g.children(":nth("+b+")").html())}}}if(f&&e.fields&&e.rows&&e.rows.length>0){for(var b in e.fields){if(e.fields[b]==f){e=e.rows[0][b];break}}}return e}function setProperty_grid_selectedRowData(k,b,l,a,g,c){gridData=getGridDataStoreData(b,a);if(!gridData){gridData={}}g=makeDataObject(g,l);if(gridData.selectedRowNumber){if(g){var n=gridData.fields;if(n){var d={};for(var f in g.fields){var l=g.fields[f];var m=false;for(var e in gridData.fields){if(l.toLowerCase()==gridData.fields[e].toLowerCase()){d[l]=e;
