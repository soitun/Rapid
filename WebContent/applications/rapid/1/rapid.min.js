...
}}},success:function(p){if(m){m(p)}if(b){b(p)}}})}function Action_validation(j,b,o){var a=true;for(var f in b){var d=b[f];var c=$("#"+d.controlId);if(c[0]){var e=null;if(d.validationProperty){e=window["getProperty_"+d.controlType+"_"+d.validationProperty]}else{e=window["getData_"+d.controlType]}if(e){var l=e(j,d.controlId,d.field,d.details);if(d.validationType=="javascript"){var h=new Function(["ev","id","value"],d.javaScript);var n=h.apply(this,[j,d.controlId,l]);if(n!=null&&n!==false&&n!==undefined){if(o){showControlValidation(d.controlId,n)}a=false}else{if(o){hideControlValidation(d.controlId)}}}else{var m=false;if(l){var k=new RegExp(d.regEx);var g=l.match(k);if(g){if(g.length==1){m=true}else{for(var f in g){if(f!="index"&&f!="input"&&g[f]==l){m=true;break}}}}}else{if(d.allowNulls){m=true}}if(m||(c.is(":hidden")&&d.passHidden)){if(o){hideControlValidation(d.controlId)}}else{if(o&&!(j.type=="keyup"&&j.keyCode==9)){showControlValidation(d.controlId,d.message)}a=false}}}else{if(o){showControlValidation(d.controlId,d.message)
}a=false}}}return a}function Action_webservice(c,f,d,e){if(d&&e){if(d.sequence>getWebserviceActionMaxSequence(f)){_webserviceActionMaxSequence[f]=d.sequence;for(var b in e){var a=e[b];window["setData_"+a.type](c,a.id,a.field,a.details,d)}}}};