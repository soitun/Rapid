...
$(this).children().each(function(q){if(b.columns[q].field){p.push($(this).html())}});f.rows.push(p)});if(b.dataStorageType){var a=getGridDataStoreData(c,b);if(a){if(f.fields.length<a.fields.length){var d={};for(var g in f.fields){for(var e in a.fields){if(f.fields[g]&&a.fields[e]&&f.fields[g].toLowerCase()==a.fields[e].toLowerCase()){d[g]=e;break}}}for(var g in f.rows){if(a.rows.length<g-1){a.rows.push([]);for(k in a.fields){a.rows[g].push(null)}}for(e in d){a.rows[g][d[e]]=f.rows[g][e]}}}f=a}}}}}}return f}function setData_grid(s,l,a,r,u,h){var d=$("#"+l);d.find("tr:not(:first-child)").remove();d.find("span.sort").html("&#xf0dc");if(!d.is("table")){d=d.find("table").last()}if(u!==undefined){u=makeDataObject(u,a);if(u&&u.rows){if(r&&r.columns&&u.fields){var b=[];for(var p in r.columns){for(var o in u.fields){var g=false;if(r.columns[p].field&&u.fields[o]){if(r.columns[p].field.toLowerCase()==u.fields[o].toLowerCase()){g=true}}else{if(!u.fields[o]){g=true}}if(g){b.push(o);break}}if(b.length==p){b.push("")
