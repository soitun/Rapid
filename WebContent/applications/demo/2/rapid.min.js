...
if(b&&b.columns){if(e){var f=$(c.target).closest("tr");var g=f.index()-1;if(g>=0){for(var a in b.columns){if(b.columns[a].field.toLowerCase()==e.toLowerCase()){d=f.children(":nth("+a+")").html();break}}}}else{var d={};d.fields=[];for(var a in b.columns){d.fields.push(b.columns[a].field)}d.rows=[];$("#"+h).find("tr:not(:first)").each(function(j){var k=[];$(this).children().each(function(l){k.push($(this).html())});d.rows.push(k)})}}return d}function setData_grid(d,h,n,a){var g=$("#"+d);g.find("tr:not(:first)").remove();if(h!=null&&h!==undefined){h=makeDataObject(h,n);if(h.rows){if(a&&a.columns&&h.fields){var m=[];for(var k in a.columns){for(var f in h.fields){var p=false;if(a.columns[k].field){if(a.columns[k].field.toLowerCase()==h.fields[f].toLowerCase()){p=true}}else{if(!h.fields[f]){p=true}}if(p){m.push(f);break}}if(m.length==k){m.push("")}if(a.columns[k].cellFunction&&!a.columns[k].f){a.columns[k].f=new Function(a.columns[k].cellFunction)}}for(var k in h.rows){var o=h.rows[k];var l=g.append("<tr class='rowStyle"+(k%2+1)+"'></tr>").find("tr:last");
...
for(var c in a){var d=a[c];if(d.type){var e=window["getData_"+d.type](null,d.itemId,d.field,d.details)}else{var e=$.getUrlVar(d.itemId)}if(e!==undefined){b+="&"+d.name+"="+e}}window.location=b}$(document).ready(function(){$(window).resize(function(b){if(typeof(window.parent._pageIframe)==="undefined"){var d=$(window);$(".slidePanelCover").css({width:d.width(),height:d.height()})}else{var c=window.parent._pageIframe;var a=window.parent._scale;$(".slidePanelCover").css({width:c.width()/a,height:c.height()/a})}})});var _databaseActionSequence=[];var _databaseActionMaxSequence=[];function getDatabaseActionSequence(b){var a=_databaseActionSequence[b];if(!a){a=0}a++;_databaseActionSequence[b]=a;return a}function getDatabaseActionMaxSequence(b){var a=_databaseActionMaxSequence[b];if(a===undefined){a=0;_databaseActionMaxSequence[b]=a}return a}var _webserviceActionSequence=[];var _webserviceActionMaxSequence=[];function getWebserviceActionSequence(b){var a=_webserviceActionSequence[b];if(!a){a=0
}a++;_webserviceActionSequence[b]=a;return a}function getWebserviceActionMaxSequence(b){var a=_webserviceActionMaxSequence[b];if(a===undefined){a=0;_webserviceActionMaxSequence[b]=a}return a};