...
}d.rows=[];for(var e in f.rows){var c=f.rows[e];var p=[];for(var b in g){var m=g[b];var n=-1;for(var a in f.fields){if(m.toLowerCase()==f.fields[a].toLowerCase()){n=a;break}}if(n>-1){p.push(c[n])}else{p.push(null)}}d.rows.push(p)}}else{d.rows=[];d.rows.push([])}}else{d.fields=[];d.rows=[];d.rows.push([]);for(var e in g){var l=g[e];d.fields.push(l.id);d.rows[0].push(l.value)}}return d}var _webserviceActionSequence=[];var _webserviceActionMaxSequence=[];function getWebserviceActionSequence(b){var a=_webserviceActionSequence[b];if(!a){a=0}a++;_webserviceActionSequence[b]=a;return a}function getWebserviceActionMaxSequence(b){var a=_webserviceActionMaxSequence[b];if(a===undefined){a=0;_webserviceActionMaxSequence[b]=a}return a};function Init_date(b,a){A_TCALCONF.format=a.dateFormat;f_tcalAdd(b)}function Init_grid(g,c){var e=$("#"+g);if(c){if(c.columns){var f=e.find("tr:first-child td");for(var b in c.columns){if(c.columns[b].sort){var a=$(f.get(b));a.css("cursor","pointer");a.click({index:b},function(i){var h=$(i.target);if(!h.is("span")){h=h.find("span")}if(h.html()=="\uf0de"){sortGridByColumn(i,g,c,i.data.index,false);h.html("&#xf0dd;")}else{sortGridByColumn(i,g,c,i.data.index,true);h.html("&#xf0de;")}})}}}if(c.dataStorageType){var d=getGridDataStoreData(g,c);if(d){setData_grid($.Event("gridinit"),g,null,c,d)}$("#"+g).click(function(h){var i=getGridDataStoreData(g,c);i.selectedRowNumber=$(h.target).closest("tr").index();saveGridDataStoreData(g,c,i)})}}}function Init_tabGroup(b,a){$("#"+b).children("ul").children("li").each(function(){var c=$("#"+b);horizontal=true;if(window[b+"details"]&&window[b+"details"].tabType=="V"){horizontal=false}if(horizontal){c.children("ul").addClass("tabsHeaderH")}else{c.children("ul").addClass("tabsHeaderV")
