...
}d.rows=[];for(var e in f.rows){var c=f.rows[e];var p=[];for(var b in g){var m=g[b];var n=-1;for(var a in f.fields){if(m.toLowerCase()==f.fields[a].toLowerCase()){n=a;break}}if(n>-1){p.push(c[n])}else{p.push(null)}}d.rows.push(p)}}else{d.rows=[];d.rows.push([])}}else{d.fields=[];d.rows=[];d.rows.push([]);for(var e in g){var l=g[e];d.fields.push(l.id);d.rows[0].push(l.value)}}return d}var _webserviceActionSequence=[];var _webserviceActionMaxSequence=[];function getWebserviceActionSequence(b){var a=_webserviceActionSequence[b];if(!a){a=0}a++;_webserviceActionSequence[b]=a;return a}function getWebserviceActionMaxSequence(b){var a=_webserviceActionMaxSequence[b];if(a===undefined){a=0;_webserviceActionMaxSequence[b]=a}return a};function Init_date(b,a){A_TCALCONF.format=a.dateFormat;f_tcalAdd(b)}function Init_grid(h,d){var f=$("#"+h);if(!f.is("table")){f=f.find("table").last()}if(d){if(d.columns){var g=f.find("tr:first-child td");for(var c in d.columns){if(d.columns[c].sort){var a=$(g.get(c));a.css("cursor","pointer");a.click({index:c},function(j){var i=$(j.target);if(!i.is("span")){i=i.find("span")}if(i.html()=="\uf0de"){sortGridByColumn(j,h,d,j.data.index,false);i.html("&#xf0dd;")}else{sortGridByColumn(j,h,d,j.data.index,true);i.html("&#xf0de;")}})}}}if(d.dataStorageType){var e=getGridDataStoreData(h,d);if(e){setData_grid($.Event("gridinit"),h,null,d,e)}$("#"+h).click(function(i){var j=getGridDataStoreData(h,d);j.selectedRowNumber=$(i.target).closest("tr").index();saveGridDataStoreData(h,d,j)})}if(d.fixedHeader){var b=f.clone(true);b.find("tr:not(:first-child)").remove();b.removeAttr("style");f.find("tr:first-child").hide();f.before(b)}}}function Init_tabGroup(b,a){$("#"+b).children("ul").children("li").each(function(){var c=$("#"+b);
...
}}}}}function setProperty_dropdown_value(c,f,e,a,d,b){var d=makeDataObject(d,e);if(d&&d.rows){$("#"+f).val(d.rows[0][0])}else{$("#"+f).val("")}}function getData_grid(f,b,g,a){var c=null;if(a){if(a.dataStorageType&&!$("#"+b)[0]){c=getGridDataStoreData(b,a);if(g&&c.fields&&c.rows&&c.rows.length>0){for(var d in c.fields){if(c.fields[d]==g){c=c.rows[0][d];break}}}}else{if(a.columns){if(g){var j=$("#"+b).find("tr.rowSelect");var e=j.index()-1;if(e>=0){for(var d in a.columns){if(a.columns[d].field.toLowerCase()==g.toLowerCase()){c=j.children(":nth("+d+")").html();break}}}}else{var c={};c.fields=[];for(var d in a.columns){var g=a.columns[d].field;if(g){c.fields.push(a.columns[d].field)}}c.rows=[];var h=$("#"+b);if(!h.is("table")){h=h.find("table").last()}h.find("tr:not(:first):not([data-ignore])").each(function(k){var l=[];$(this).children().each(function(m){if(a.columns[m].field){l.push($(this).html())}});c.rows.push(l)})}}}}return c}function setData_grid(v,n,a,u,y,m){var f=$("#"+n);f.find("tr:not(:first-child)").remove();
f.find("span.sort").html("&#xf0dc");if(!f.is("table")){f=f.find("table").last()}if(y!==undefined){y=makeDataObject(y,a);if(y&&y.rows){if(u&&u.columns&&y.fields){var d=[];for(var s in u.columns){for(var q in y.fields){var k=false;if(u.columns[s].field){if(u.columns[s].field.toLowerCase()==y.fields[q].toLowerCase()){k=true}}else{if(!y.fields[q]){k=true}}if(k){d.push(q);break}}if(d.length==s){d.push("")}if(u.columns[s].cellFunction&&!u.columns[s].f){u.columns[s].f=new Function(u.columns[s].cellFunction)}}for(var s in y.rows){var e=y.rows[s];var h=f.append("<tr class='rowStyle"+(s%2+1)+"'></tr>").find("tr:last");for(var q in u.columns){var t="";if(!u.columns[q].visible){t+="display:none;"}if(u.columns[q].style){t+=u.columns[q].style}if(t){t=" style='"+t+"'"}var p="";var o=d[q];if(o&&e[o]){p=e[d[q]]}var g=h.append("<td"+t+">"+p+"</td>").find("td:last");if(u.columns[q].f){u.columns[q].f.apply(g,[n,y,a,u])}}if(y.rowValidation&&s<y.rowValidation.length){if(!y.rowValidation[s]){h.addClass("validation")
}}}if(y.selectedRowNumber){f.find("tr:nth("+y.selectedRowNumber+")").addClass("rowSelect")}}else{for(var s in y.rows){var e=y.rows[s];var w="<tr>";for(var q in e){w+="<td>"+e[q]+"</td>"}w+="</tr>";f.append(w)}}}if(u&&u.dataStorageType){saveGridDataStoreData(n,u,y)}if(u&&u.fixedHeader){var r=false;if(!f.is(":visible")){r=true;f.parent().show()}var x=f.prev().find("tr:first-child").children();x.css("width","");var b=f.find("tr:nth-child(2)").children();if(b.length>0){for(var s=0;s<b.length;s++){var c=$(x.get(s));var l=$(b.get(s));if(c.width()<l.width()){c.width(l.width())}if(c.width()>l.width()){l.width(c.width())}}}if(r){f.parent().hide()}}f.children().last().children("tr:not(:first)").click(function(){var i=$(this);i.parent().find("tr.rowSelect").removeClass("rowSelect");i.addClass("rowSelect")})}}function getProperty_grid_selectedRowData(d,h,f,c){var e=null;if(c.dataStorageType){e=getGridDataStoreData(h,c);e.rows=[e.rows[e.selectedRowNumber-1]]}else{var g=$(d.target).closest("tr");var a=g.index();
if(a>0){e={fields:[],rows:[[]]};for(var b in c.columns){e.fields.push(c.columns[b].field);e.rows[0].push(g.children(":nth("+b+")").html())}}}if(f&&e.fields&&e.rows&&e.rows.length>0){for(var b in e.fields){if(e.fields[b]==f){e=e.rows[0][b];break}}}return e}function setProperty_grid_selectedRowData(k,b,l,a,g,c){gridData=getGridDataStoreData(b,a);if(!gridData){gridData={}}g=makeDataObject(g,l);if(gridData.selectedRowNumber){if(g){var n=gridData.fields;if(n){var d={};for(var f in g.fields){var l=g.fields[f];var m=false;for(var e in gridData.fields){if(l.toLowerCase()==gridData.fields[e].toLowerCase()){d[l]=e;m=true;break}}if(!m){gridData.fields.push(l);d[l]=gridData.fields.length-1}}for(var f in g.fields){if(gridData.rows[gridData.selectedRowNumber-1].length<g.fields.length){gridData.rows[gridData.selectedRowNumber-1].push(null)}gridData.rows[gridData.selectedRowNumber-1][d[g.fields[f]]]=g.rows[0][f]}}else{gridData.fields=g.fields;gridData.rows[gridData.selectedRowNumber-1]=g.rows[0]}}else{gridData.rows.splice(gridData.selectedRowNumber-1,1);
