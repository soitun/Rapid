...
}d.rows=[];for(var e in f.rows){var c=f.rows[e];var p=[];for(var b in g){var m=g[b];var n=-1;for(var a in f.fields){if(m.toLowerCase()==f.fields[a].toLowerCase()){n=a;break}}if(n>-1){p.push(c[n])}else{p.push(null)}}d.rows.push(p)}}else{d.rows=[];d.rows.push([])}}else{d.fields=[];d.rows=[];d.rows.push([]);for(var e in g){var l=g[e];d.fields.push(l.id);d.rows[0].push(l.value)}}return d}var _webserviceActionSequence=[];var _webserviceActionMaxSequence=[];function getWebserviceActionSequence(b){var a=_webserviceActionSequence[b];if(!a){a=0}a++;_webserviceActionSequence[b]=a;return a}function getWebserviceActionMaxSequence(b){var a=_webserviceActionMaxSequence[b];if(a===undefined){a=0;_webserviceActionMaxSequence[b]=a}return a};function Init_date(b,a){A_TCALCONF.format=a.dateFormat;f_tcalAdd(b)}function Init_grid(h,d){var f=$("#"+h);if(d){if(d.columns){var g=f.find("tr:first-child td");for(var c in d.columns){if(d.columns[c].sort){var a=$(g.get(c));a.css("cursor","pointer");a.click({index:c},function(j){var i=$(j.target);if(!i.is("span")){i=i.find("span")}if(i.html()=="\uf0de"){sortGridByColumn(j,h,d,j.data.index,false);i.html("&#xf0dd;")}else{sortGridByColumn(j,h,d,j.data.index,true);i.html("&#xf0de;")}})}}}if(d.dataStorageType){var e=getGridDataStoreData(h,d);if(e){setData_grid($.Event("gridinit"),h,null,d,e)}$("#"+h).click(function(i){var j=getGridDataStoreData(h,d);j.selectedRowNumber=$(i.target).closest("tr").index();saveGridDataStoreData(h,d,j)})}if(d.fixedHeader){var b=f.clone(true);f.find("tr:first-child").hide();b.removeAttr("id");b.find("tr:not(:first-child)").remove();b.appendTo(f.parent().prev())}}}function Init_tabGroup(b,a){$("#"+b).children("ul").children("li").each(function(){var c=$("#"+b);horizontal=true;
