...
f+=o.text()+", ";i++;m=true;break}o.removeClass("listSelected")}});if(i>0){f=f.substring(0,f.length-2);if(a.multi&&a.slide){d.text(i).show()}}else{f=a.header;d.hide()}g.html(f)}else{g.html(a.header);d.hide()}}}function getData_radiobuttons(b,d,c,a){return $("#"+d).children("input[type=radio]:checked").val()}function setData_radiobuttons(h,b,j,a,e,c){if(e!=null&&e!==undefined){var g=$("#"+b);g.children("input[type=radio]").prop("checked",false);e=makeDataObject(e,j);var k=null;if(e.rows&&e.rows[0]){if(j&&e.fields){for(var d in e.fields){if(e.fields[d].toLowerCase()==j.toLowerCase()){k=e.rows[0][d];break}}}else{k=e.rows[0][0]}}if(k){var f=g.children("input[type=radio][value="+k+"]");if(f[0]){f.prop("checked",true);if(c){f.trigger("change")}}}}}function getData_text(b,d,c,a){return $("#"+d).html()}function setData_text(d,h,g,b,e,c){var f=$("#"+h);if(e!=null&&e!==undefined){e=makeDataObject(e,g);if(e.rows&&e.rows[0]){if(g&&e.fields){for(var a in e.fields){if(e.fields[a]&&e.fields[a].toLowerCase()==g.toLowerCase()){f.html(e.rows[0][a]);
