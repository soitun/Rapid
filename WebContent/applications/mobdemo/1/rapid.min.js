...
var f=g.data;if(j){var l=-1;for(var e in f.fields){if(j==f.fields[e]){return f.rows[0][e]}}return null}else{return f}}else{return null}}function getData_radiobuttons(b,d,c,a){return $("#"+d).children("input[type=radio]:checked").val()}function setData_radiobuttons(h,b,j,a,e,c){if(e!=null&&e!==undefined){var g=$("#"+b);g.children("input[type=radio]").prop("checked",false);e=makeDataObject(e,j);var l=null;if(e.rows&&e.rows[0]){if(j&&e.fields){for(var d in e.fields){if(e.fields[d].toLowerCase()==j.toLowerCase()){l=e.rows[0][d];break}}}else{l=e.rows[0][0]}}if(l!==undefined){var f=g.children("input[type=radio][value='"+l+"']");if(f[0]){f.prop("checked",true)}else{g.children("input[type=radio]").prop("checked",false)}if(c){f.trigger("change")}}}}function getData_text(b,d,c,a){return $("#"+d).html()}function setData_text(d,h,g,b,e,c){var f=$("#"+h);if(e!=null&&e!==undefined){e=makeDataObject(e,g);if(e.rows&&e.rows[0]){if(g&&e.fields){for(var a in e.fields){if(e.fields[a]&&e.fields[a].toLowerCase()==g.toLowerCase()){f.html(e.rows[0][a]);
