...
var _webserviceActionMaxSequence=[];function getWebserviceActionSequence(b){var a=_webserviceActionSequence[b];if(!a){a=0}a++;_webserviceActionSequence[b]=a;return a}function getWebserviceActionMaxSequence(b){var a=_webserviceActionMaxSequence[b];if(a===undefined){a=0;_webserviceActionMaxSequence[b]=a}return a};function Init_date(b,a){A_TCALCONF.format=a.dateFormat;f_tcalAdd(b)}function Init_gallery(b,a){$("#"+b).children("img").click(function(c){Gallery_removeImage(c,b)})}function Init_grid(g,c){var e=$("#"+g);if(c){if(c.columns){var f=e.find("tr:first-child td");for(var b in c.columns){if(c.columns[b].sort){var a=$(f.get(b));a.css("cursor","pointer");a.append("<span class='sort'>&#xf0dc;</span>");a.click({index:b},function(i){var h=$(i.target);if(!h.is("span")){h=h.find("span")}if(h.html()=="\uf0de"){sortGridByColumn(i,g,c,i.data.index,false);h.html("&#xf0dd;")}else{sortGridByColumn(i,g,c,i.data.index,true);h.html("&#xf0de;")}})}}}if(c.dataStorageType){var d=getGridDataStoreData(g,c);if(d){setData_grid($.Event("gridinit"),g,null,c,d)}$("#"+g).click(function(h){var i=getGridDataStoreData(g,c);i.selectedRowNumber=$(h.target).closest("tr").index();saveGridDataStoreData(g,c,i)})}}}function Init_hints(f,d){var a=$("body");for(var c in d.controlHints){var b=d.controlHints[c];if(!$("#"+b.controlId+"hint")[0]){var e=b.style;
