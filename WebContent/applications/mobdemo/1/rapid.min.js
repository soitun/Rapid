...
break}}}if(f==null&&b.dataStorageType){var a=getGridDataStoreData(c,b);if(a.rows>h){for(var g in a.fields){if(a.fields[g]&&a.fields[g].toLowerCase()==m.toLowerCase()){f=a.rows[h][g];break}}}}}else{var f={};f.fields=[];for(var g in b.columns){var m=b.columns[g].field;if(m){f.fields.push(b.columns[g].field)}}f.rows=[];var n=$("#"+c);if(!n.is("table")){n=n.find("table").last()}n.find("tr:not(:first):not([data-ignore])").each(function(j){var p=[];$(this).children().each(function(q){if(b.columns[q].field){p.push($(this).html())}});f.rows.push(p)});if(b.dataStorageType){var a=getGridDataStoreData(c,b);if(f.fields.length<a.fields.length){var d={};for(var g in f.fields){for(var e in a.fields){if(f.fields[g]&&a.fields[e]&&f.fields[g].toLowerCase()==a.fields[e].toLowerCase()){d[g]=e;break}}}for(var g in f.rows){if(a.rows.length<g-1){a.rows.push([]);for(k in a.fields){a.rows[g].push(null)}}for(e in d){a.rows[g][d[e]]=f.rows[g][e]}}}f=a}}}}}return f}function setData_grid(s,l,a,r,u,h){var d=$("#"+l);
d.find("tr:not(:first-child)").remove();d.find("span.sort").html("&#xf0dc");if(!d.is("table")){d=d.find("table").last()}if(u!==undefined){u=makeDataObject(u,a);if(u&&u.rows){if(r&&r.columns&&u.fields){var b=[];for(var p in r.columns){for(var o in u.fields){var g=false;if(r.columns[p].field&&u.fields[o]){if(r.columns[p].field.toLowerCase()==u.fields[o].toLowerCase()){g=true}}else{if(!u.fields[o]){g=true}}if(g){b.push(o);break}}if(b.length==p){b.push("")}if(r.columns[p].cellFunction&&!r.columns[p].f){r.columns[p].f=new Function(["id","data","field","details","value"],r.columns[p].cellFunction)}}for(var p in u.rows){var c=u.rows[p];var f=d.append("<tr class='rowStyle"+(p%2+1)+"'></tr>").find("tr:last");for(var o in r.columns){var q="";if(!r.columns[o].visible){q+="display:none;"}if(r.columns[o].style){q+=r.columns[o].style}if(q){q=" style='"+q+"'"}var n="";var m=b[o];if(m&&c[m]){n=c[b[o]]}var e=f.append("<td"+q+">"+n+"</td>").find("td:last");if(r.columns[o].f){r.columns[o].f.apply(e,[l,u,a,r,n])
}}if(u.rowValidation&&p<u.rowValidation.length){if(!u.rowValidation[p]){f.addClass("validation")}}}if(u.selectedRowNumber){d.find("tr:nth("+u.selectedRowNumber+")").addClass("rowSelect")}}else{for(var p in u.rows){var c=u.rows[p];var t="<tr>";for(var o in c){t+="<td>"+c[o]+"</td>"}t+="</tr>";d.append(t)}}}if(r&&r.dataStorageType){saveGridDataStoreData(l,r,u)}if(r&&r.fixedHeader){sizeGridFixedHeaderColumns(d)}d.children().last().children("tr:not(:first)").click(function(){var i=$(this);i.parent().find("tr.rowSelect").removeClass("rowSelect");i.addClass("rowSelect")})}}function getProperty_grid_selectedRowData(f,b,g,a){var c=null;if(a.dataStorageType){c=getGridDataStoreData(b,a);if(c.selectedRowNumber){c.rows=[c.rows[c.selectedRowNumber-1]]}else{c.rows=[]}}else{var j=$(f.target).closest("tr");var e=j.index();if(e>0){c={fields:[],rows:[[]]};for(var d in a.columns){c.fields.push(a.columns[d].field);c.rows[0].push(j.children(":nth("+d+")").html())}}}if(g&&c.fields&&c.rows&&c.rows.length>0){var h=false;
for(var d in c.fields){if(c.fields[d]&&c.fields[d].toLowerCase()==g.toLowerCase()){c=c.rows[0][d];h=true;break}}if(!h){c=null}}return c}function setProperty_grid_selectedRowData(l,b,m,a,g,c){gridData=getGridDataStoreData(b,a);if(!gridData){gridData={}}g=makeDataObject(g,m);if(gridData.selectedRowNumber){if(g&&g.fields&&g.rows&&g.rows.length>0){var o=gridData.fields;if(o){var d={};for(var f in g.fields){var m=g.fields[f];var n=false;for(var e in gridData.fields){if(m.toLowerCase()==gridData.fields[e].toLowerCase()){d[m]=e;n=true;break}}if(!n){gridData.fields.push(m);d[m]=gridData.fields.length-1}}for(var f in g.fields){if(gridData.rows[gridData.selectedRowNumber-1].length<g.fields.length){gridData.rows[gridData.selectedRowNumber-1].push(null)}gridData.rows[gridData.selectedRowNumber-1][d[g.fields[f]]]=g.rows[0][f]}}else{gridData.fields=g.fields;gridData.rows[gridData.selectedRowNumber-1]=g.rows[0]}}else{gridData.rows.splice(gridData.selectedRowNumber-1,1);if(gridData.rowValidation){gridData.rowValidation.splice(gridData.selectedRowNumber-1,1)
