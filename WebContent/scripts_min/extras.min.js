...
break}}}if(u<f.rows.length&&q==null){for(var r in f.fields){if(p[t].toLowerCase()==f.fields[r].toLowerCase()){q=f.rows[u][r];break}}}g.push(q)}z.rows.push(g)}}break;case"child":var d={};var y=0;for(var u in f.fields){for(var t in c.fields){if(f.fields[u].toLowerCase()==c.fields[t].toLowerCase()){d[u]=t;y++}}}var p=c.fields;if(y>0){f.fields.push(b);for(var u in f.rows){var m=false;var w=f.rows[u];for(var t in c.rows){var s=c.rows[t];var a=0;for(var r in d){if(w[r]==s[d[r]]){a++}}if(a==y){m=true;var e;if(f.rows[u].length<f.fields.length){e={fields:p,rows:[]}}else{e=f.rows[u][f.fields.length-1]}var g=c.rows[t];e.rows.push(g);w.push(e)}}if(!m){w.push(null)}}}else{if(f.rows&&f.rows.length>0){var x=-1;for(var u in f.fields){if(b.toLowerCase()==f.fields[u].toLowerCase()){x=u;break}}if(x<0){f.fields.push(b);f.rows[0].push(c)}else{f.rows[0][x]=c}}}z=f;break;case"search":var x=0;if(b){for(var u in c.fields){if(b.toLowerCase()==c.fields[u].toLowerCase()){x=u;break}}}var q=f.rows[0][0];if(q){q=q.toLowerCase()
}var z={fields:c.fields,rows:[]};for(var u in c.rows){var o=c.rows[u][x];if(typeof o!=="undefined"){if(o.toLowerCase().indexOf(q)>-1){z.rows.push(c.rows[u])}}}break}}else{z=f}}else{z=c}return z}function toPixels(a){if(a){if(a.indexOf("em")==a.length-2){var b=parseFloat($("body").css("font-size"));return(b*parseFloat(a))}else{return parseFloat(a)}}else{return 0}};