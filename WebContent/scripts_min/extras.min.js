...
break}}if(h==-1){a.push(c);f.push(c);h=f.length-1}}}else{a.push(c);f.push(c);h=f.length-1}m[h]=l[c];if(h<f.length-1){h++}}}else{if(e==0){f.push(g)}m=[l]}n.push(m)}}else{var m=[];if($.isPlainObject(d)){for(var e in d){f.push(e);m.push(d[e])}}else{f.push(g);m.push(d)}n.push(m)}d={fields:f,rows:n}}return d}function mergeDataObjects(l,d,o,c,a){var D=null;if(l){l=makeDataObject(l);if(d){d=makeDataObject(d);switch(o){case"append":case"row":var s=[];for(var z in l.fields){s.push(l.fields[z])}for(var z in d.fields){var n=false;for(var x in s){if(d.fields[z].toLowerCase()==s[x].toLowerCase()){n=true;break}}if(!n){s.push(d.fields[z])}}D={};D.fields=s;if(o=="append"){D.rows=l.rows;for(var z=0;z<d.rows.length;z++){var m=[];for(var x in s){var t=null;for(var u in d.fields){if(s[x].toLowerCase()==d.fields[u].toLowerCase()){t=d.rows[z][u];break}}m.push(t)}D.rows.push(m)}}else{D.rows=[];var q=d.rows.length;if(l.rows.length>q){q=l.rows.length}for(var z=0;z<q;z++){var m=[];for(var x in s){var t=null;if(z<d.rows.length){for(var u in d.fields){if(s[x].toLowerCase()==d.fields[u].toLowerCase()){t=d.rows[z][u];
break}}}if(z<l.rows.length&&t==null){for(var u in l.fields){if(s[x].toLowerCase()==l.fields[u].toLowerCase()){t=l.rows[z][u];break}}}m.push(t)}D.rows.push(m)}}break;case"child":var e={};var C=0;for(var z in l.fields){for(var x in d.fields){if(l.fields[z].toLowerCase()==d.fields[x].toLowerCase()){e[z]=x;C++}}}var s=d.fields;if(C>0){l.fields.push(c);for(var z in l.rows){var p=false;var y=l.rows[z];for(var x in d.rows){var w=d.rows[x];var b=0;for(var u in e){if(y[u]==w[e[u]]){b++}}if(b==C){p=true;var h;if(l.rows[z].length<l.fields.length){h={fields:s,rows:[]}}else{h=l.rows[z][l.fields.length-1]}var m=d.rows[x];h.rows.push(m);y.push(h)}}if(!p){y.push(null)}}}else{if(l.rows&&l.rows.length>0){var B=-1;for(var z in l.fields){if(c.toLowerCase()==l.fields[z].toLowerCase()){B=z;break}}if(B<0){l.fields.push(c);l.rows[0].push(d)}else{l.rows[0][B]=d}}}D=l;break;case"search":var g=[];if(c){for(var A in s.split(",")){A=A.trim();if(A){for(var z in d.fields){if(d.fields[z]&&d.fields[z].toLowerCase()==A.toLowerCase()){g.push(z)
}}}}}var D={fields:d.fields,rows:[]};var t=l.rows[0][0];if(t&&g.length>0){t=t.toLowerCase();for(var z in d.rows){for(var x in g){var r=d.rows[z][g[x]];if(typeof r!=="undefined"){if(r.toLowerCase().indexOf(t)>-1){D.rows.push(d.rows[z]);break}}}if(D.rows.length>=a){break}}}break}}else{D=l}}else{D=d}return D}function toPixels(a){if(a){if(a.indexOf("em")==a.length-2){var b=parseFloat($("body").css("font-size"));return(b*parseFloat(a))}else{return parseFloat(a)}}else{return 0}};